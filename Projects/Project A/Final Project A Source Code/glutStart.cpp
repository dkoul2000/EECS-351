...
int isCleared = 1, helpButton = 0, count = 0;                  //  animation's screen-clear
...
// First, draw the square with NO transformations:
    ...
    if (count % 100 > 50)
    {
        glColor3d(0.0, 0.0, 0.0);// And Cyan-colored text on-screen:
        drawText2D(helv18, 0.4, 0.7,"CYAN SCREEN!!");
        glClearColor(0, 1, 1, 0);
    }

    else
    {
        glColor3d(1.0, 0.0, 0.0);// And Cyan-colored text on-screen:
        drawText2D(helv18, 0.4, 0.7,"BLACK SCREEN!!");
...
//    glLoadIdentity();
    ...
    glRotated(theta1, 1, 1, 1);
    glTranslated(-0.7, 0.3, 0.2);
    glutSolidCylinder(0.08, 0.5, 50, 30);
...
//    glLoadIdentity();
    ...
    glTranslated(-0.25, 0.31, 0);
    glutSolidCylinder(0.08, 0.9, 50, 30);

    glLoadIdentity();
    glColor3d(1.0, 1.0, 1.0);
    glRotated(theta1, 1, 1, 1);
    glTranslated(0.45, 0.35, 0.2);
    glutSolidCylinder(0.08, 0.50, 50, 30);
    ...
    glLoadIdentity();
    glRotated(theta1, 0, 1, 0);
    glTranslated(-0.65, 0.40, 0);
    glColor3d(1, 0, 1);
    glutSolidCube((GLdouble) 0.20);
...
void printInstructions()
{
    glLoadIdentity();
    glColor3d(1.0, 0.0, 0.0);
    drawText2D(helv12, -0.5, -0.6, "Click and drag the mouse to see it react angrily!");
    drawText2D(helv12, -0.5, -0.65, "Click the mouse repeatedly to see a flashing cone rotate each click.");
    drawText2D(helv12, -0.5, -0.7, "Press 'S' to STOP the motion of the graphics window.");
    drawText2D(helv12, -0.5, -0.75, "Press 'C' to see CRAZY things happen on-screen!");
    drawText2D(helv12, -0.5, -0.8, "(Press the 'Space bar', ESC, 'Q' or 'q' to quit!)");
    //drawText2D(helv18, -0.2, 0.48, "HELP INSTRUCTIONS");//Press the SPACE BAR, or 'Q' to QUIT.");
    //glFlush();
    //glutSwapBuffers();
}
...
int xpos,ypos;  // mouse position in coords with origin at lower left.
	...
	switch(key)
	{
		...
		case 'q':
			...
			break;
        default:
            cout << "that's not the 'C' or 'S' key!\n";
            glColor3d(1.0, 0.0, 0.0);// And Cyan-colored text on-screen:
...
////////////GOT THIS CODE FROM AN ONLINE RESOURCE (DETAILS IN THE REPORT)
...
int xpos,ypos;  // mouse position in coords with origin at lower left.
    ...
    glColor3d(1.0, 0, 0);	//bright red text:

    drawText2D(rom24, -2, 0, "STOP DRAGGING THE MOUSE!!!");
...
void askForHelp()
{
    glLoadIdentity();
    glColor3d(1.0, 1.0, 1.0);
    drawText2D(helv18,-0.65,-0.7, "Press 'H' for instructions on how to run the program.");
    //glutSwapBuffers();
}
