...
//          -- UniformColorVertShad.vsh, UniformColorFragShad.fsh sets all
//              vertex colors by'uniform' shader vars r_shad, g_shad, b_shad.
//              note both the vertex shader and frag shader can read these.
//      --Added r,g,b/R,G,B key to adjust r_app, g_app, b_app values sent to
//          'uniform' vertex shader values r_shad, g_shad, b_
...
//  glClear( GL_COLOR_BUFFER_BIT)'; ?)
    ...
    p_myGLSL = new CProgGLSL(argv[0],   "UniformColorVertShad.vsh",
                                        "UniformColorFragShad.fsh");
...
int xpos,ypos;  // mouse position in coords with origin at lower left.
	...
	switch(key)
	{
        ...
        case 'S':
            ...
            break;
        //========================Shader Controls===============================
        case 'r':       // increment a 'uniform' value sent to prog. shaders.
            r_app += 0.1;
            if(r_app > 1.0) r_app = 1.0; // stay within 0.0 - 1.0.
            glUniform1f(r_shadID, r_app);
                // send the value to the GPU's 'uniform' float variable 'r_shad'
                // for use in the vertex shader and fragment shader.
            cout << "r_app: " << r_app << endl;
            break;
        case 'R':       // decrement.
            r_app -= 0.1;
            if(r_app < 0.0) r_app = 0.0;
            glUniform1f(r_shadID, r_app);
                // send the value to the GPU's 'uniform' float variable 'r_shad'
                // for use in the vertex shader and fragment shader.            break;
            cout << "r_app: " << r_app << endl;
            break;
        case 'g':       //' increment a 'uniform' value sent to prog. shaders.
            g_app += 0.1;
            if(g_app > 1.0) g_app = 1.0;
            glUniform1f(g_shadID, g_app);
                // send the value to the GPU's 'uniform' float variable 'g_shad'
                // for use in the vertex shader and fragment shader.
            cout << "g_app: " << g_app << endl;
            break;
        case 'G':       // decrement.
            g_app -= 0.1;
            if(g_app < 0.0) g_app = 0.0;
            glUniform1f(g_shadID, g_app);
                // send the value to the GPU's 'uniform' float variable 'g_shad'
                // for use in the vertex shader and fragment shader.
            cout << "g_app: " << g_app << endl;
            break;
        case 'b':       // increment a 'uniform' value sent to prog. shaders.
            b_app += 0.1;
            if(b_app > 1.0) b_app = 1.0;
            glUniform1f(b_shadID, b_app);
                // send the value to the GPU's 'uniform' float variable 'b_shad'
                // for use in the vertex shader and fragment shader.
            cout << "b_app: " << b_app << endl;
            break;
        case 'B':
            b_app -= 0.1;
            if(b_app < 0.0) b_app = 0.0;
            glUniform1f(b_shadID, b_app);
                // send the value to the GPU's 'uniform' float variable 'b_shad'
                // for use in the vertex shader and fragment shader.
            cout << "b_app: " << b_app << endl;
