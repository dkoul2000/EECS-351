...
//	drawAxes();			            // draw simple right-handed +x,+y,+z axes
    ...
    glPushMatrix();             // save current matrix, then
        ...
        glPushMatrix();
            ...
            if(isSolid==1)  glutSolidCylinder(0.1, 0.3, 33, 23);
            else            glutWireCylinder(0.1, 0.3, 33, 23);
        glPopMatrix();
        ...
        glPushMatrix();                     // SAVE current matrix, then
            ...
            if(isCleared==1)            // if user wants to see robot, then
            {
                ...
                if(isSolid==1) glutSolidCylinder(1.0, 2.0, 47, 31);   // solid
                else            glutWireCylinder(1.0, 2.0, 47, 31);   // wireframe
            }
