...
2012.02.22  Jack Tumblin;
    added 'getProgramID(),getVertShaderID(), getFragShaderID()' to retrieve
    these private members for use in glGetUniformLocation() and other GLSL calls
    Added printout of GLSL 'info logs' when GLSL compile and link steps fail.
*/
...
//------------------------------------------------------------------------------
{
#define JT_LOG_MAXLENGTH 8096
GLchar myLogBuf[JT_LOG_MAXLENGTH];
GLsizei myLogLength;
GLint ans;
...
GLint vertCompiled,fragCompiled, progLinked;
    ...
    if(!vertCompiled)
    {
        ...
        // retrieve the 'info log' generated by the compiler:
        glGetShaderInfoLog(vSh.shaderObjID, JT_LOG_MAXLENGTH,
                           &myLogLength, myLogBuf);

        cout << "\n Error Info Log:" << endl;
        cout <<"-----------------------------------------------" << endl;
        cout << myLogBuf << endl;
        cout <<"----------(end Error Info Log)-----------------" << endl;
        exit(EXIT_FAILURE);
    }
    if(!fragCompiled)
    {
        ...
        cout <<                     "] did not compile!" << endl;
        // retrieve the 'info log' generated by the compiler:
        glGetShaderInfoLog(fSh.shaderObjID, JT_LOG_MAXLENGTH,
                           &myLogLength, myLogBuf);

        cout << "\n Shader Error Info Log: ("<< myLogLength << " chars)"<< endl;
        cout <<"------------------------------------------------------" << endl;
        cout << myLogBuf << endl;
        cout <<"----------(end Shader error Info Log)-----------------" << endl;
        exit(EXIT_FAILURE);
    }
    // Now let's link the entire GLSL program; it holds our 2 compiled shaders
    glLinkProgram(programID);
    // Check for errors:
    glGetProgramiv(programID, GL_LINK_STATUS, &progLinked);
    if(!progLinked)
    {   // Report them.
        cout << "GLSL Program (holds vert & frag shaders) DID NOT LINK!"<< endl;
        glGetProgramInfoLog(programID,JT_LOG_MAXLENGTH, &myLogLength, myLogBuf);
        cout << "\n Program Error Info Log: ("<< myLogLength <<" chars)"<< endl;
        cout <<"-------------------------------------------------------"<< endl;
        cout << myLogBuf << endl;
        cout <<"----------(end Program Error Info Log)-----------------"<< endl;
        exit(EXIT_FAILURE);
    }
    cout << " GLSL Program linking info: " << endl;
    cout << "------------------------------" << endl;
    glGetProgramiv(programID, GL_ATTACHED_SHADERS,  &ans);
    cout << "\t" << ans << " attached shaders found," << endl;
    glGetProgramiv(programID, GL_ACTIVE_ATTRIBUTES, &ans);
    cout << "\t" << ans << " active attributes found," << endl;
    glGetProgramiv(programID, GL_ACTIVE_UNIFORMS, &ans);
    cout << "\t" << ans << " active uniforms found," << endl << endl;
    glGetProgramInfoLog(programID, JT_LOG_MAXLENGTH, &myLogLength, myLogBuf);
    cout << "\n Program Linking Info Log: ("<< myLogLength <<" chars)"<< endl;
    cout <<"---------------------------------------------------------"<< endl;
    cout << myLogBuf << endl;
    cout <<"----------(end Program Linking Info Log)-----------------" << endl;

    // and we'll (by default) say we want to use this new program
    // by calling CProgGLSL::useProgram()
}
...
GLuint CProgGLSL::getProgramID(void)
//------------------------------------------------------------------------------
// return GPU's 'program' ID# that identifies our the GLSL program object.
{                           // (needed because 'programID' is a private member)
    return programID;
}

GLuint CProgGLSL::getVertShaderID(void)
//------------------------------------------------------------------------------
// returns GPU's 'shader' ID# that identifies our vertex-shader object.
{                           // (needed because vSh is a private member)
    return vSh.shaderObjID;
}
GLuint CProgGLSL::getFragShaderID(void)
//------------------------------------------------------------------------------
// returns GPU's 'shader' ID# that identifies our fragment shader object.
{                           // (needed because fSh is a private member)
    return fSh.shaderObjID;
}
