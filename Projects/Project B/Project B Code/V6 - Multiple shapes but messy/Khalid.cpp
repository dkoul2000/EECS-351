...
//==============================================================================
{
	...
	glutCreateWindow( "EECS351-1 Project Cube" ); // set window title-bar name
...
int xpos,ypos;      // mouse position in coords with origin at lower left.
	...
	switch(key)
	{
        case GLUT_KEY_UP:       // User pressed the 'A' key...
            heightAng -= heightAngInc;
            if (heightAng < 25)
                heightAng = 25;
            //cout << "that *IS* the 'A' key!\n";
            break;
		case GLUT_KEY_DOWN:       // User pressed the 'A' key...
            heightAng += heightAngInc;
            if (heightAng > 145)
                heightAng = 145;
            //cout << "that *IS* the 'A' key!\n";
...
void Ccube::drawSolid( double height )
//------------------------------------------------------------------------------
// Use openGL calls to draw the cube as a solid object:
{

    // assign vertices
	vertex[0][0] = -1; vertex[0][1] = -1; vertex[0][2] = 0; // lower left back
	vertex[1][0] = -1; vertex[1][1] = -1; vertex[1][2] = height;  // lower left front
	vertex[2][0] = 1;  vertex[2][1] = -1; vertex[2][2] = height;  // lower right front
	vertex[3][0] = 1;  vertex[3][1] = -1; vertex[3][2] = 0; // lower right back
	vertex[4][0] = -1; vertex[4][1] = 1;  vertex[4][2] = 0; // upper left back
	vertex[5][0] = -1; vertex[5][1] = 1;  vertex[5][2] = height;  // upper left front
	vertex[6][0] = 1;  vertex[6][1] = 1;  vertex[6][2] = height;  // upper right front
	vertex[7][0] = 1;  vertex[7][1] = 1;  vertex[7][2] = 0; // upper right back
