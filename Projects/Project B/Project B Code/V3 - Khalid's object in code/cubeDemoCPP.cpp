...
pyramid pyr = pyramid();
int rotateValue = 0, rotateIncrement = 5;
...
//    ice.drawSolid(2.0);
    glPushMatrix();
        glTranslated(0,0,3);
        glRotated(180,1,0,0);
        glRotated(rotateValue,0,0,1);
        pyr.draw(1);
        glPushMatrix();
            glTranslated(0,1,0);
            glRotated(45-rotateValue*2,0,0,1);
...
void printInstructions()
{
    ...
    drawText2D(helv12, -0.5, -1.1, "Click the mouse repeatedly to see a flashing cone rotate each click.");
    drawText2D(helv12, -0.5, -1.3, "Press 'S' to STOP the motion of the graphics window.");
    drawText2D(helv12, -0.5, -1.5, "Press 'C' to see CRAZY things happen on-screen!");
...
object::object(double x, double y, int numVerts)
{

    xPos = x;
    yPos = y;
    pColors = new triple[numVerts];

    srand(time(NULL)); // color vertices
    for(int i = 0; i < numVerts; i++) {
        pColors[i][0] = ((double) (rand() % 10))/10;
        pColors[i][1] = ((double) (rand() % 10))/10;
        pColors[i][2] = ((double) (rand() % 10))/10;
    }
...
void pyramid::draw(int height) {
  ...
  glBegin(GL_TRIANGLE_STRIP);
    ...
    glColor3f(0, 1, 0); glVertex3f(0, 0, height);
    glColor3f(0, 0, 1); glVertex3f(-1, 0, 0);
    glColor3f(1, 1, 1); glVertex3f(0, 1, 0);
    glColor3f(0, 1, 0); glVertex3f(0, 0, height);
    glColor3f(1, 1, 1); glVertex3f(1, 0, 0);
